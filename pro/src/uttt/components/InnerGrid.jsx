...
const InnerGrid = ({ _gridIndex, gridData, macroWinner, isActive, isLastMoveGrid, lastMoveSquare, onSquareClick }) => {
  const getContainerClasses = () => {
    ...
    if (macroWinner) {
      const bgColor = macroWinner === 'X' ? 'bg-blue-50/80 border-blue-200/50' : 'bg-red-50/80 border-red-200/50';
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          <div className="grid grid-cols-3 gap-0.5 opacity-30">
            {Array(9)
              .fill(null)
              .map((_, i) => (
                <span key={i} className="text-[6px] sm:text-[8px] md:text-xs text-gray-500 font-bold">
                  ×
                </span>
              ))}
          </div>
