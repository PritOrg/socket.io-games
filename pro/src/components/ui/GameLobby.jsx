import React, { useState, useEffect, useRef } from 'react';
import SketchButton from './SketchButton';
import AvatarSelector from './AvatarSelector';
import CountdownOverlay from './CountdownOverlay';

const GameLobby = ({
  gameName = 'Game',
  roomId,
  players = [],
  spectators = [],
  isHost,
  minPlayers = 2,
  canStart,
  onStart,
  onLeave,
  onJoinSubmit,
  showJoinInput = false,
  customizationSlot,
  startLabel = 'Start Game',
  waitingLabel = 'Waiting for players…',
  showCountdown = false,
  onCountdownComplete,
  profile = { name: '', avatarIcon: 'cat', color: '#2a2a3e' },
  onProfileChange,
}) => {
  const [dots, setDots] = useState('');
  const [shareSupported] = useState(() => !!navigator.share);
  const [copied, setCopied] = useState(false);
  const joinInputRef = useRef(null);

  const actuallyCanStart = canStart ?? players.length >= minPlayers;

  useEffect(() => {
    if (players.length >= minPlayers) return;
    const id = setInterval(() => {
      setDots((d) => (d.length >= 3 ? '' : d + '.'));
    }, 500);
    return () => clearInterval(id);
  }, [players.length, minPlayers]);

  const handleShare = async () => {
    try {
      await navigator.share({
        title: `Join my ${gameName} room!`,
        text: `Use room code: ${roomId}`,
        url: window.location.href,
      });
    } catch {
      // user cancelled or not supported
    }
  };

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(roomId);
      setCopied(true);
      setTimeout(() => setCopied(false), 1500);
    } catch {
      // fallback or error
    }
  };

  const handleProfileChange = (field, value) => {
    if (onProfileChange) {
      onProfileChange({ ...profile, [field]: value });
    }
  };

  return (
    <div className="min-h-screen bg-paper flex items-center justify-center p-4">
      {showCountdown && <CountdownOverlay onComplete={onCountdownComplete} />}

      <div className="sketch-card p-8 max-w-sm w-full space-y-6">
        <h2 className="text-2xl font-sketch text-center text-ink">{gameName}</h2>

        <div className="text-center">
          <p className="text-sm text-ink/60 paper-font">Room Code</p>
          <p className="text-3xl font-sketch text-ink tracking-widest">{roomId}</p>
          <SketchButton onClick={handleCopy} className="mt-2 text-sm">
            {copied ? '✓ Copied!' : '📋 Copy Room Code'}
          </SketchButton>
        </div>

        {customizationSlot && <div className="space-y-2 pt-2 border-t border-ink/10">{customizationSlot}</div>}

        <div className="space-y-2">
          <p className="paper-font text-sm text-ink/60">Player Name (optional)</p>
          <input
            type="text"
            value={profile.name}
            onChange={(e) => handleProfileChange('name', e.target.value)}
            placeholder="Enter your name"
            className="w-full px-3 py-2 border border-ink/30 rounded sketch-font text-ink bg-white/80"
          />
        </div>

        <AvatarSelector
          avatarIcon={profile.avatarIcon}
          color={profile.color}
          onAvatarChange={(icon) => handleProfileChange('avatarIcon', icon)}
          onColorChange={(color) => handleProfileChange('color', color)}
        />

        <div
          className="sketch-border text-center py-3"
          style={{ animation: actuallyCanStart ? 'none' : 'sketch-wiggle 2s ease-in-out infinite' }}
        >
          {actuallyCanStart ? (
            <span className="paper-font text-green-700 font-semibold">✅ Ready to start!</span>
          ) : (
            <span className="paper-font text-ink/70">
              {waitingLabel || 'Waiting for players'}
              {dots}
            </span>
          )}
        </div>

        <div className="space-y-2">
          <p className="paper-font text-sm text-ink/60">
            Players ({players.length}/{minPlayers} min)
          </p>
          {players.map((p, i) => (
            <div
              key={p.id || i}
              className="flex items-center gap-2 player-badge"
              style={{ transform: 'none', border: 'none', padding: '0.25rem 0' }}
            >
              <span className="text-lg">{p.connected !== false ? '☑️' : '☐'}</span>
              <span className="paper-font text-ink">{p.name}</span>
              {i === 0 && <span className="text-xs text-ink/50 ml-auto paper-font">host</span>}
            </div>
          ))}
          {Array.from({ length: Math.max(0, minPlayers - players.length) }).map((_, i) => (
            <div key={`empty-${i}`} className="flex items-center gap-2" style={{ opacity: 0.4 }}>
              <span className="text-lg">☐</span>
              <span className="paper-font text-ink/50">Waiting...</span>
            </div>
          ))}
        </div>

        {spectators.length > 0 && (
          <div className="space-y-2 pt-2 border-t border-ink/10">
            <p className="paper-font text-sm text-ink/60">Spectators ({spectators.length})</p>
            {spectators.map((s, i) => (
              <div key={s.id || i} className="flex items-center gap-2" style={{ padding: '0.25rem 0' }}>
                <span className="text-lg">👁️</span>
                <span className="paper-font text-ink/70">{s.name}</span>
              </div>
            ))}
          </div>
        )}

        {showJoinInput && onJoinSubmit && (
          <div className="space-y-2">
            <p className="paper-font text-sm text-ink/60">Join Existing Room</p>
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Enter Room ID"
                className="flex-1 px-3 py-2 border border-ink/30 rounded sketch-font text-ink bg-white/80"
                id="join-room-input"
                ref={joinInputRef}
              />
              <SketchButton
                onClick={() => {
                  if (joinInputRef.current?.value) onJoinSubmit(joinInputRef.current.value.trim().toUpperCase());
                }}
                className="text-sm"
              >
                Join
              </SketchButton>
            </div>
          </div>
        )}

        <div className="space-y-2">
          {shareSupported && (
            <SketchButton onClick={handleShare} className="w-full">
              📤 Invite Friends
            </SketchButton>
          )}

          {showJoinInput && <p className="paper-font text-sm text-ink/50 text-center">Enter room ID above to join</p>}

          {isHost && onStart && (
            <SketchButton
              onClick={onStart}
              disabled={!actuallyCanStart}
              className="w-full"
              style={{ opacity: actuallyCanStart ? 1 : 0.5, cursor: actuallyCanStart ? 'pointer' : 'not-allowed' }}
            >
              {actuallyCanStart ? `🚀 ${startLabel}` : `Need ${minPlayers - players.length} more player(s)`}
            </SketchButton>
          )}

          {!isHost && onStart && (
            <p className="paper-font text-center text-sm text-ink/50">Waiting for host to start…</p>
          )}

          <SketchButton onClick={onLeave} className="w-full" style={{ background: '#fff0f0' }}>
            Leave Room
          </SketchButton>
        </div>
      </div>
    </div>
  );
};

export default GameLobby;
