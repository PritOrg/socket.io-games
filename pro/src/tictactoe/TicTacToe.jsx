...
const TicTacToe = () => {
  ...
  useEffect(() => {
    ...
    socket.on(
      "ttt_roomInfo",
      ({ id, players, gameState, currentTurn, board }) => {
        ...
        if (playerIndex !== -1) {
          ...
          sessionStorage.setItem(
            "ttt_reconnect",
            JSON.stringify({ roomId: id, playerId: players[playerIndex].id }),
          );
        }
      },
    );
      ...
      <div className="grid grid-cols-3 gap-1 sm:gap-2 w-48 sm:w-64 md:w-80">
        {board.map((cell, i) => (
          <button
            ...
            disabled={
              gameState !== "playing" || currentTurn !== socket?.id || cell
            }
            className={`
              ...
              {players.map((p, i) => (
                ...
                >
                  {p.id === currentTurn ? (
                    <Users
                      size={12}
                      className="sm:w-4 text-ink animate-pulse"
                    />
                  ) : (
                    <div
